...
# gcp = gcsfs.GCSFileSystem(project = 'project-dev-midgard')
...
class ClassificationModel:
    ...
    def train_model(
        ...
        **kwargs,
        ...
        """
        Trains the model using 'full_data'

        Args:
            full_data: Pandas Dataframe containing at least two columns. If the Dataframe has a header, it should contain a 'text' and a 'labels' column. If no header is present,
        ...
        # def d_append(dict_, key, value):
        #     if key not in dict_:
        #         dict_[key] = []
        #     dict_[key].append(value)

        # increment = [0.5, 0.65, 0.8, 0.95, 1]
        ...
        # train_size = 0.8
        # train_df = fullset.sample(frac=train_size,random_state=200).reset_index(drop=True)
        ...
        logger.info(" ---- Deleting training example ---- ")
        del(train_examples)
        gc.collect()
        ...
        **kwargs,
        ...
        if args["fp16"]:
            try:
                ...
                logger.info("Installing apex library...")
                os.system("cd ./resources")
                os.system("git clone https://github.com/NVIDIA/apex.git")
                os.system("cd apex && pip install -v --no-cache-dir ./ && cd ../..")
                from apex.apex import amp
        ...
        for _ in train_iterator:
            ...
            if args["evaluate_during_training"]:
                results, _, _ = self.eval_model(
                    eval_df, verbose=verbose and args["evaluate_during_training_verbose"], silent=False, **kwargs
                )
    ...
    def eval_model(self, eval_df, multi_label=False, output_dir=None, verbose=True, silent=False, **kwargs):
        ...
        # evaluation_progress_scores = self._create_training_progress_scores(multi_label, **kwargs)

        # for key in self.results:
        #     evaluation_progress_scores[key].append(self.results[key])



        # if self.args["wandb_project"]:
        #     wandb.log(self._get_last_metrics(evaluation_progress_scores))
    ...
    def evaluate(self, eval_df, output_dir, multi_label=False, prefix="", verbose=True, silent=False, **kwargs):
        ...
        logger.info(" ---- Deleting evaluation example ---- ")
        del(eval_examples)
        gc.collect()
        ...
        with open(output_eval_file, "w") as writer:
            writer.write("===============================================================" + "\n")
            writer.write("===========Benchmark for {} Baseline Classifier===========".format(self.args["model_type"]) + "\n")
            writer.write("===============================================================" + "\n")
            for key in sorted(results.keys()):
                writer.write(str(datetime.datetime.now().time()) + " " + "{} = {}\n\n".format(key, str(results[key])))
        # with open(output_eval_file, "w") as writer:
    ...
    def compute_metrics(self, preds, labels, eval_examples, multi_label=False, **kwargs):
        ...
        # thresh = 0.50
        # predicted = np.array([[1 if i > thresh else 0 for i in j] for j in preds])
        # # predicted = (preds >= t).astype(int)

        predicted = np.array(preds) >= 0.5
        ...
        # f1_ = f1_score(labels, predicted, average='micro')

        returned = confusion_matrix(labels, preds, labels=[0, 1])
        cm = returned.astype('float') / returned.sum(axis=1)[:, np.newaxis]

        label_dict = {0: 'non_title',
                        1: 'title'}

        class_wise_acc = list(zip([label_dict[k] for k in [0, 1]],cm.diagonal().tolist()))

        if multi_label:
            label_ranking_score = label_ranking_average_precision_score(labels, preds)
            return {**{"LRAP": label_ranking_score,"F1-Score": f1, "Accuracy": accuracy}, "Report": classification_report, **extra_metrics}, wrong # , "Accuracy": accuracy, "F1-Score": f1, "Precision": precision, "Recall": recall, "F1-Score": f1
        ...
        if self.model.num_labels == 2:
            ...
            return (
                {**{"Class-wise Accuracy": class_wise_acc, "Accuracy": accuracy, "Precision": precision, "Recall": recall, "F1-Score": f1, "mcc": mcc,}, **extra_metrics},
                wrong,
            ) # , "Precision": precision, "Recall": recall, "F1-Score": f1
        else:
            return {**{"Class-wise Accuracy": class_wise_acc, "Accuracy": accuracy, "Precision": precision, "Recall": recall, "F1-Score": f1, "mcc": mcc,}, **extra_metrics}, wrong #, "tp": tp, "tn": tn, "fp": fp, "fn": fn
